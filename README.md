...
- `powershell -ExecutionPolicy Bypass -File check.ps1` — полный контроль качества: **pyflakes** (линт) + **mypy** (типы) + **pytest** (78 тестов). Запускать перед коммитом.
- `venv\Scripts\python.exe -m pytest` — только модульные тесты.
- `venv\Scripts\python.exe -m mypy rulens` — только проверка типов (конфиг в `mypy.ini`).
